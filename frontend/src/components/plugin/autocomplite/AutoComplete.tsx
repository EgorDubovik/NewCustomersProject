...
const AutoComplete = ({ children, inputValue, list, onInputChange, onSaggestionClick }: AutoCompleteProps): ReactElement => {
	...
	return (
		<div className="relative">
			{returnInput()}
			<div className={'absolute ' + (!showSuggestions ? 'hidden' : '') + ' top-10 bg-gray-100 left-0 w-full dark:bg-zinc-900 z-50 rounded shadow-lg overflow-hidden'}>
				<ul className="list">
					{suggestions.map((item: any, index: number) => (
						<li
							key={index}
							className={
								'p-2 px-4 border-b dark:border-gray-700 cursor-pointer last:border-0 hover:bg-gray-300 hover:dark:text-primary hover:dark:bg-zinc-950 flex justify-between ' +
								(index === selectedSuggestion ? 'dark:bg-zinc-950 dark:text-primary' : '')
							}
