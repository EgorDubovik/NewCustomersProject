...
const Employees = () => {
	...
	const saveEmployee = () => {
		if (dataForm.id) {
			...
			axiosClient
				.put(`/employees/${dataForm.id}`, dataForm)
				.then((res) => {
					setEmployees(
					...
					alertSuccsess('Employee updated successfully');
					setModal(false);
				})
				.catch((err) => {
					console.log(err);
			...
			setLoadingDataForm(true);
			axiosClient
				.post('/employees', dataForm)
				.then((res) => {
					setEmployees([...employees, res.data.employee]);
					alertSuccsess('Employee added successfully');
					setModal(false);
				})
				.catch((err) => {
					console.log(err);
												...
												<tr key={employee.id}>
													...
													<td>
														{employee.rolesArray.map((role: number, index: number) => (
															<span key={index} className={`badge badge-outline-${rolesColor[role]} ml-2`}>
																{rolesTitle[role]}
															</span>
														))}
													</td>
												...
												<div className="ml-2 w-full overflow-hidden">
													<div className="user-info">
														<div className="user-name w-full font-bold dark:text-white">
															{employee.name}
															{employee.rolesArray.map((role: number, index: number) => (
																<span key={index} className={`badge badge-outline-${rolesColor[role]} ml-2 text-[10px]`} onClick={() => chooseRole(index)}>
																	{rolesTitle[role]}
																</span>
															))}
														</div>
										...
										<form>
											...
											<div className="mb-5">
												...
												{rolesTitle.map((role: string, index: number) => {
													...
													return (
														<span
															key={index}
															className={`badge cursor-pointer ${dataForm.rolesArray.includes(index) ? 'bg' : 'badge-outline'}-${rolesColor[index]} ml-4`}
											...
											<div className="choose-color mb-5 flex justify-center items-center">
												<div className="">
													<span
														className="flex justify-center items-center w-10 h-10 text-center rounded-full object-cover bg-'bg-danger text-white"
														style={{ backgroundColor: dataForm.color }}
													>
														{getTechAbr(dataForm.name)}
													</span>
												</div>
												<div className="ml-10">
													<CirclePicker
