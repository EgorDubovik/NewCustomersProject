...
const SelectDateTime = () => {
	...
	const selectDayHandle = (day: number, isActive: boolean) => {
		if (isActive) setSelectedDate(new Date(viewDate.getFullYear(), viewDate.getMonth(), day));
	};
						...
						{days.map((day, index) => {
							return (
								<div
									key={index}
									onClick={() => {
										selectDayHandle(day.day, day.isActive);
									}}
									className={
										'date text-center p-2 text-sm ' +
										(day.isActive ? ' cursor-pointer hover:bg-blue-500 hover:rounded hover:text-white' : 'text-gray-300 cursor-default') +
										' ' +
										(day.isSelected ? 'bg-blue-500 text-white font-bold rounded' : '')
									}
								>
									{day.day}
								</div>
						...
						{timeArray.map((time, index) => {
							return (
								<div
									key={index}
									onClick={() => {
										selectTimeHandle(time.value, time.isActive);
									}}
									className={
										'p-2 rounded px-4 text-center ' +
										(time.isActive ? ' bg-white border border-blue-300 text-blue-600 hover:bg-blue-500 cursor-pointer hover:text-white hover:font-bold' : 'text-gray-400')
									}
								>
									{time.title}
								</div>
