...
const CustomerInfo = () => {
	...
	const sendInfo = () => {
		...
		fetch(import.meta.env.VITE_API_URL + '/appointment/book/' + key, {
			method: 'POST',
			headers: {
			...
			body: JSON.stringify({
				...
				problemDescription: customerInfo.problemDescription,
				...
				selectedDateTime:
					...
					':' +
					((selectedDateTime?.getMinutes() || 0 <= 9 ? '0' : '') + selectedDateTime?.getMinutes()) +
					':00',
			}),
		})
			.then((response) => {
			...
			.catch((error) => {
				...
				alertError('An error occurred while booking the appointment.');
			})
			.finally(() => {
				setLoading(false);
			});
	...
	return (
		<div className="col-span-2 text-left p-2 relative items-center block">
			...
			<div className={loading ? 'blur-sm' : ''}>
				...
				<div className="pt-4">
					<div className="grid grid-cols-1 gap-y-6 p-3 px-6 bg-white rounded-lg shadow-lg">
						<div className="relative h-11 w-full ">
							<input
								...
								onChange={onChange}
								placeholder=""
								className="peer h-full w-full border-b border-gray-400 bg-transparent pt-4 pb-1.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-blue-500 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50 placeholder:opacity-0 focus:placeholder:opacity-100"
							/>
						...
						<div className="grid grid-cols-3 gap-6">
							...
							<div className="problem-desc col-span-3">
							...
							<div className="col-span-3">
								<div className="pt-3 text-right">
									<button
										...
										type="button"
										className="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 font-medium rounded-md text-sm px-5 py-3 me-2 mb-2"
									>
										Create appointment
									</button>
								</div>
			...
			{loading && (
				<div role="status" className="absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 z-20">
					<svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-blue-200 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path
